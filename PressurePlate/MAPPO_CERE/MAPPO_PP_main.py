...
class Runner_MAPPO_SMAC:
    ...
    def evaluate_policy(self, evaluate_num):
        ...
        for _ in range(self.args.evaluate_times):
            win_tag, episode_reward, _, episode_step, _ = self.run_episode_smac(evaluate=True, t_s=self.total_steps)
            if win_tag:
                win_times += 1
    ...
    def run_episode_smac(self, evaluate=False, current_avg_ir=-1, t_s=0):
        ...
        for episode_step in range(self.args.episode_limit):
            ...
            a_n, a_logprob_n, entropy_log = self.agent_n.choose_action_ts(obs_n, avail_a_n,
...
if __name__ == '__main__':
    ...
    for _ in range(6):
        ...
        for env_index in range(len(env_names)):
            runner = Runner_MAPPO_SMAC(args, env_name=env_names[env_index], number=number, seed=seed_value)
            runner.run()
