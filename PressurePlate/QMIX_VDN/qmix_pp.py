...
class QMIX_SMAC(object):
    ...
    def calc_entropy(self, avail_a_n, epsilon):
        if isinstance(avail_a_n, torch.Tensor):
            avail_tensor = avail_a_n.detach().clone()
        elif isinstance(avail_a_n, np.ndarray):
            avail_tensor = torch.from_numpy(avail_a_n).detach().clone()
        else:
