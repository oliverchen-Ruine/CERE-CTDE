...
def run(_config, run_number=0):
    ...
    unique_token = "{}__{}_{}".format(args.ALGORITHM, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"),
                                      args.MAP)
    args.unique_token = unique_token
    if args.use_tensorboard:
        tb_logs_direc = os.path.join(dirname(dirname(abspath(__file__))), "results", "tb_logs_sim")
...
def run_sequential(args, logger):
    ...
    # while train_iteration <= args.TRAIN_STEP_MAX:
        ...
        if args.EPSILON_UPDATE_STANDARD == "steps":
            mac.action_selector.update_epsilon(steps=runner.t_env)
            if args.EXPLORATION:
                exp_mac.update_noise_epsilon(steps=runner.t_env)
