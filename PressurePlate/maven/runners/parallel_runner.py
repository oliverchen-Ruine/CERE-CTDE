...
class ParallelRunner:
    ...
    def reset(self, test_mode=False):
        ...
        if "map_name" in self.args.env_args and self.args.env_args["map_name"] == "2_corridors":
            if self.t_env > self.args.t_max / 2:
                for parent_conn in self.parent_conns:
    ...
    def run(self, test_mode=False, test_uniform=False):
        ...
        episode_win_nums = [0 for _ in range(self.batch_size)]
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    if data["terminated"] and not data["info"].get("episode_limit", False):
                        env_terminated = True
                    if data["terminated"]:
                        if test_mode and'battle_won' in data["info"] and data["info"]['battle_won'] and data["info"]["battle_won"]:
                            episode_win_nums[idx] += 1
        ...
        if test_mode:
            cur_stats["win_rate"] = sum(episode_win_nums) + cur_stats.get("win_rate", 0)
    ...
    def _log(self, returns, stats, prefix):
        ...
        for k, v in stats.items():
            if k != "n_episodes" and k != "avail_actions":
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step":
            actions = data
            if env.map_name ==  'linear-4p':
                # Take a step in the environment
                all_obs, reward, terminated, env_info = env.step(actions)
            ...
            if env.map_name ==  'linear-4p':
                all_obs, state, env_info = env.reset()
                remote.send({
                    "state": state,
                    "avail_actions": env_info['avail_actions'],
                    "obs": all_obs
