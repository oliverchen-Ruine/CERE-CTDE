...
class MAPPO_SMAC:
    ...
    def compute_masked_entropy(self, normalized_probs, avail_a_n, eps=1e-10):
        """
        Calculate the entropy values considering the action mask and the live state

        parameters:
        - raw_probs: The original action probability, shaped as (N, action_dim)
        - action_mask: The action mask (0= invalid, 1= valid) has the same shape as raw_probs
        - alive_mask: Survival status mask (True= alive, False= dead), in the shape of (N,)
        - eps: Minimum value, preventing division by zero errors

        return:
        - mean_entropy: The average entropy value (scalar) of the surviving agent
        """
        ...
        for _ in range(self.K_epochs):
            for index in BatchSampler(SequentialSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                if self.use_value_clip:
                    ...
                    #  直接对新旧价值预测的差异进行裁剪，然后计算裁剪后的价值预测与目标价值之间的差异
                    #  v - (r + gamma*v')
