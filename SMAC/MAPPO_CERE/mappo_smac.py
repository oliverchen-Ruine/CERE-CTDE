...
class MAPPO_SMAC:
    def __init__(self, args):
        ...
        self.rnd_input_dim = args.rnd_dim
        ...
        self.rnd = RND(obs_shape=self.rnd_input_dim, device=args.device)
        ...
        self.ts = ThompsonSampling(self.N, self.action_dim)

    def compute_masked_entropy(self, normalized_probs, avail_a_n, eps=1e-10):
        entropy = - (normalized_probs * torch.log(normalized_probs + eps)).sum(dim=-1)  # shape (N,)
        ...
        alive_mask = torch.tensor(alive_mask)
        alive_entropy = entropy[alive_mask]

        step_mean_entropy = alive_entropy.mean().item() if len(alive_entropy) > 0 else 0.0

        return step_mean_entropy, alive_mask.sum().item()

    def choose_action_ts(self, obs_n, avail_a_n, evaluate):
        with torch.no_grad():
            actor_inputs = []
            # obs_n contains numpy.ndarray
            obs_n = torch.tensor(np.array(obs_n).astype(np.float32))  # obs_n.shape=(N，obs_dim)
            ...
            if evaluate:  # When evaluating the policy, we select the action with the highest probability
                ...
                # Here, the Thompson sampling operation needs to be performed
                a_n = self.ts.sample_action(probs=prob.numpy())
                a_logprob_n = self.ts.calculate_log_prob(probs=prob, actions=a_n)
                # entropy, num_alive = self.compute_masked_entropy(prob, avail_a_n)
                # return a_n, a_logprob_n, (entropy, num_alive)
                return a_n, a_logprob_n

    def get_value(self, s, obs_n):
        with torch.no_grad():
            critic_inputs = self.get_value_input(s, obs_n)
            v_n = self.critic(critic_inputs)  # v_n.shape(N,1)
            return v_n.numpy().flatten()

    def get_intrinsic_reward(self, s, obs_n, time_step, current_avg_ir):
        rnd_inputs = self.get_value_input(s, obs_n)
        # Adapt to the SMAC reward environment
        if current_avg_ir == -1:
            return self.rnd.adjust_smac_ir(self.rnd.compute_irs(rollouts={"obs": rnd_inputs}, time_steps=time_step))
        return self.rnd.adjust_smac_ir_v2(current_avg_ir,
                                          self.rnd.compute_irs(rollouts={"obs": rnd_inputs}, time_steps=time_step),
                                          [0, 2])

    def get_value_input(self, s, obs_n):
        value_input = []
        ...
        value_input.append(s)
        if self.use_agent_specific:  # Add local obs of agents
            value_input.append(torch.tensor(np.array(obs_n).astype(np.float32)))
        if self.add_agent_id:  # Add a one-hot vector to represent the agent_id
            value_input.append(torch.eye(self.N))
        value_input = torch.cat([x for x in value_input], dim=-1)  # value_inputs.shape=(N, value_input_dim)
        return value_input
    ...
    def train(self, replay_buffer, total_steps):
        ...
        for _ in range(self.K_epochs):
            for index in BatchSampler(SequentialSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                """
                if self.use_rnn:
                    # If we use RNN, we need to reset the rnn_hidden of the actor and critic.
                ...
                a_logprob_n_now = dist_now.log_prob(
                    batch['a_n'][index])  # a_logprob_n_now.shape=(mini_batch_size, max_episode_len, N)
                # a/b=exp(log(a)-log(b))
                ratios = torch.exp(a_logprob_n_now - batch['a_logprob_n'][
                    index].detach())  # ratios.shape=(mini_batch_size, max_episode_len, N)
    ...
    def get_inputs(self, batch, max_episode_len):
        ...
        if self.add_agent_id:
            ...
            agent_id_one_hot = torch.eye(self.N).unsqueeze(0).unsqueeze(0).repeat(self.batch_size, max_episode_len, 1,
                                                                                  1)
            actor_inputs.append(agent_id_one_hot)
            critic_inputs.append(agent_id_one_hot)

        actor_inputs = torch.cat([x for x in actor_inputs],
                                 dim=-1)  # actor_inputs.shape=(batch_size, max_episode_len, N, actor_input_dim)
        critic_inputs = torch.cat([x for x in critic_inputs],
                                  dim=-1)  # critic_inputs.shape=(batch_size, max_episode_len, N, critic_input_dim)
        return actor_inputs, critic_inputs

    def save_model(self, env_name, number, seed, total_steps):
        torch.save(self.actor.state_dict(),
                   "./model/MAPPO_env_{}_actor_number_{}_seed_{}_step_{}k.pth".format(env_name, number, seed,
                                                                                      int(total_steps / 1000)))

    def load_model(self, env_name, number, seed, step):
        self.actor.load_state_dict(torch.load(
            "./model/MAPPO_env_{}_actor_number_{}_seed_{}_step_{}k.pth".format(env_name, number, seed, step)))
