...
class QMIX_SMAC(object):
    ...
    def calc_entropy(self, avail_a_n, epsilon):
        # Calculation of effective action count
        if isinstance(avail_a_n, list):
            avail_tensor = torch.tensor(avail_a_n, dtype=torch.float32)
        else:
            avail_tensor = avail_a_n.clone().detach()
        num_avail = avail_tensor.sum(dim=1, keepdim=True)  # [N,1]
        # Calculate the policy entropy of each agent
        p_g = (1 - epsilon) + epsilon / num_avail
        p_o = epsilon / num_avail
        entropy = -p_g * np.log(p_g) - (num_avail - 1) * p_o * np.log(p_o)

        # Filter out death agents
        alive_mask = []
        for _ in avail_a_n:
            if _[0] == 1:
    ...
    def choose_action_entropy(self, obs_n, last_onehot_a_n, avail_a_n, epsilon):
        ...
        with torch.no_grad():
            ...
            entropy, num_alive = self.calc_entropy(avail_a_n, epsilon)
