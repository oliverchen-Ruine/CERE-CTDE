...
class Runner_QMIX_SMAC:
    def __init__(self, args, env_name, number, seed):
        ...
        self.number = number
        self.seed = seed
        self.last_evaluate = [0, 0]  # The results of the assessment for the first two reward values are obtained
        # Set random seed
        ...
        # The dimensions of RND n_agents * (state_dim + obs_dim)
    ...
    def run(self, ):
        ...
        while self.total_steps < self.args.max_train_steps:
            ...
            _, _, eval_entropy, episode_steps = self.run_episode_smac(evaluate=False)  # Run an episode
    ...
    def evaluate_policy(self, evaluate_num):
        ...
        for _ in range(self.args.evaluate_times):
            win_tag, episode_reward, _, _ = self.run_episode_smac(evaluate=True)
            if win_tag:
                win_times += 1
        ...
        if evaluate_num > 2 and self.args.use_TS:
            if self.last_evaluate[0] - self.last_evaluate[1] > 0 and self.last_evaluate[1] - evaluate_reward > 0:
                self.agent_n.reset()
        self.last_evaluate[0] = self.last_evaluate[1]
        self.last_evaluate[1] = evaluate_reward
        ...
        self.writer.add_scalar('evaluate_reward_{}'.format(self.env_name), evaluate_reward,
                               global_step=self.total_steps)
        # Save the win rates
        np.save('./data_train/{}_env_{}_number_{}_seed_{}.npy'.format(self.args.algorithm, self.env_name, self.number,
                                                                      self.seed), np.array(self.win_rates))
    ...
    def run_episode_smac(self, evaluate=False):
        ...
        for episode_step in range(self.args.episode_limit):
            ...
            a_n, entropy_log = self.agent_n.choose_action(obs_n, last_onehot_a_n, avail_a_n, evaluate)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--use_agent_specific", type=bool, default=True,
    ...
    for _ in range(6):
        ...
        for env_index in range(len(env_names)):
            runner = Runner_QMIX_SMAC(args, env_name=env_names[env_index], number=1, seed=seed_value)
            runner.run()
