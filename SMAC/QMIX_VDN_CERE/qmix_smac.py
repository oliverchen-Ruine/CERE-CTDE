...
from MAPPO_CERE.thompson_sampling import ThompsonSampling
...
class QMIX_SMAC(object):
    ...
    def choose_action(self, obs_n, last_onehot_a_n, avail_a_n, evaluate):
        with torch.no_grad():
            inputs = []
            ...
            if evaluate:
                ...
                a_n = self.ts.sample_action_value(q_value=q_value.numpy(), avail_actions=avail_a_n)
                prob = self.ts.softmax(q_value.numpy())
                entropy, num_alive = self.compute_masked_entropy(torch.from_numpy(prob), avail_a_n)
                return a_n, (entropy, num_alive)

    def get_intrinsic_reward(self, s, obs_n, time_step):
        rnd_inputs = self.get_rnd_input(s, obs_n)
    ...
    def get_rnd_input(self, s, obs_n):
        value_input = []
