...
class SC2(MultiAgentEnv):
    ...
    def __init__(self, **kwargs):
        ...
        if self._map_info.pathing_grid.bits_per_pixel == 1 and self.map_type != '3step':
            ...
            vals_1 = np.unpackbits(np.array(np.array(list(self._map_info.pathing_grid.data)), dtype=np.uint8)).reshape(-1, 8)
            self.pathing_grid = ~np.invert(np.flip(np.transpose(np.array(
                vals_1, dtype=np.bool_).reshape(
                self.map_y, self.map_x)), axis=1)).T[::-1].T
        else:
            self.pathing_grid = np.invert(np.flip(np.transpose(np.array(
                list(self._map_info.pathing_grid.data), dtype=np.bool_).reshape(
