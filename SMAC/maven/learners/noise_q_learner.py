...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        target_mac_out = target_mac_out.masked_fill(avail_actions[:, 1:] == 0, -9999999)  # 非原地版本  # From OG deepmarl
        ...
        discrim_target = batch["noise"][:, 0].long().detach().max(dim=1)[1].unsqueeze(1).repeat(1,
                                                                                                target_repeats).reshape(
            -1)
        ...
        assert mac_out._version == 0, "张量已被非法修改！"
        assert target_mac_out._version == 0, "目标网络输出被修改！"
        loss.backward()
